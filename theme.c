	...
	if (!f) {
		...
		while (!feof(f)) {
			...
			for (i = 0; i < 11; i++) {
				...
				themes[count].color[i].r = (int)(themes[count].color[i].r /
											256.0 * 1000);
				themes[count].color[i].g = (int)(themes[count].color[i].g /
											256.0 * 1000);
				themes[count].color[i].b = (int)(themes[count].color[i].b /
											256.0 * 1000);
			}
			count++;
		}
...
void initialize_colors(theme theme)
{
	...
	// create colors
	for (i = 0; i < 11; i++) {
		init_color(theme.color[i].id, theme.color[i].r, theme.color[i].g,
				   theme.color[i].b);
